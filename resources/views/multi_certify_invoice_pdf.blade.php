...
<head>
    <meta charset="UTF-8">
    <title>Factures Certifiées Multiples</title>
    <style>
        * {
...
<body>
    @foreach($invoices as $invoice)
        @php
            ...
            $paymentLabel = match ((int) ($invoice->payment_type ?? $invoice->payment_method ?? 1)) {
                2 => 'Cheque',
                3 => 'Versement',
                4 => 'Virement',
                default => 'Espece',
            ...
            $amountLetter = $invoice->amountLetter ?? '';
        ...
        <div class="row contacts">
            <div class="col invoice-to">
                <div class="info-row"><span class="lbl">Facture :</span> {{ $factureRef }}</div>
                <div class="info-row"><span class="lbl">Mode de paiement :</span> {{ $paymentLabel }}</div>
                @if($paymentLabel === 'Cheque' && $chequeLabel !== '')
                    <div class="info-row"><span class="lbl">Chèque N° :</span> {{ $chequeLabel }}</div>
                @endif
                <div class="info-row"><span class="lbl">Patient :</span> {{ $clientName !== '' ? $clientName : '-' }}</div>
                <div class="info-row"><span class="lbl">Adresse :</span> {{ $clientAddress }}</div>
                @if($clientProfession !== '' && $clientProfession !== null && $clientProfession !== NULL && $clientProfession !== 'NULL')
                    <div class="info-row"><span class="lbl">Activité :</span> {{ $clientProfession }}</div>
                @endif
                <div class="info-row"><span class="lbl">Sétif le :</span> {{ $invoiceDate }}</div>
            </div>
