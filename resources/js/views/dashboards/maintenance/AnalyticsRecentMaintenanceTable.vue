...
const getRecentMaintenances = () => {
  ...
  dashboard.getRecentMaintenances({
    perPage: rowPerPage.value,
    currentPage: currentPage.value,
  }).then(response => {
    props.maintenancesList.value = response.data.maintenances
...
watch(() => maintenancesList, (newMaintenance) => {
  if (newMaintenance) {
    maintenancesList.value = newMaintenance
...
const paginationData = computed(() => {
  const firstIndex = maintenancesList.value.length ? (currentPage.value - 1) * rowPerPage.value + 1 : 0
  const lastIndex = maintenancesList.value.length + (currentPage.value - 1) * rowPerPage.value
...
<template>
  <VCard v-if="projects">
    <VCardItem class="project-header d-flex flex-wrap justify-space-between py-4 gap-4">
      <VCardTitle>Recent Maintenances</VCardTitle>
      <div
        class="me-3"
        style="width: 80px;"
      >
...
<!--              />-->
      ...
      <tbody>
        <tr
          v-for="maintenance in maintenancesList"
          :key="maintenance.id"
          style="height: 3.5rem;"
        >
          <!-- 👉 Individual checkbox -->
...
<!--              />-->
          ...
          <td>
            <div class="d-flex align-center gap-3">
              ...
              >
<!--                <VImg-->
<!--                  v-if="maintenance.logo.length"-->
<!--                  :src="maintenance.logo"-->
<!--                />-->
<!--                <span-->
<!--                  v-else-->
<!--                  class="font-weight-semibold"-->
<!--                >{{ avatarText(maintenance.count) }}</span>-->
              </VAvatar>
              ...
              <div>
                ...
                <span class="text-disabled">{{ maintenance.component.asset.name }}</span>
              </div>
            </div>
          </td>
          ...
          <td class="text-center">
            {{ daysLeft(maintenance.next_maintenance_date) }} DAYS
          </td>
      ...
      <!-- 👉 table footer  -->
      <tfoot v-show="!maintenancesList.length">
        <tr>
          <td
            colspan="8"
